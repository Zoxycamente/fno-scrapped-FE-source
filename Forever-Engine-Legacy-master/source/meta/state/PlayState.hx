...
class PlayState extends MusicBeatState
{
	...
	private var youtubePositionBar:Float = 0;
	...
	public static var terceiroplayer:Character;
	...
	private var opponentIndex:Int = 0;
	...
	public static var cunota:String;
	...
	public var camera3:Bool = false;
	public var thirdExists:Bool = false;

	public var stringArrow:String = '';
	...
	override public function create()
	{
		...
		if (daSong == "dupla-dinamica")
		{
			terceiroplayer = new Character().setCharacter(20, 850, "vermelho");
			add(terceiroplayer);
			thirdExists = true;
			terceiroplayer.dance();
		}
	...
	{
		var stringPinto:String = '';
		var altString:String = '';

		var baseString = 'sing' + UIStaticArrow.getArrowFromNumber(coolNote.noteData).toUpperCase();
		var testString = 'sing' + UIStaticArrow.getArrowFromNumber(coolNote.noteData);
	...
	override public function update(elapsed:Float)
	{
		...
		if (!inCutscene)
		{
			...
			else
			{
				...
				if (curTime < 0)
					curTime = 0;
				youtubePositionBar = (curTime / songLength);
							...
							if (!daNote.isSustainNote)
							{
								...
								switch (daNote.noteType)
								{
									case 1:
										if (thirdExists)
										{
											terceiroplayer.playAnim(stringArrow, true);
											terceiroplayer.holdTimer = 0;
										}
									case 2:
									// dps eu programo isso
									default:
										vocals.volume = 0;
	...
	private function charactersDance(curBeat:Int)
	{
		...
		if (thirdExists)
		{
			if ((terceiroplayer.animation.curAnim.name.startsWith("idle") || terceiroplayer.animation.curAnim.name.startsWith("dance"))
			&& (curBeat % 2 == 0 || terceiroplayer.characterData.quickDancer))
			terceiroplayer.dance();
		}
	}
	...
	override function beatHit()
	{
		...
		if (curSong.toLowerCase() == 'dupla-dinamica')
		{
			switch(curBeat)
			{
				case 16:
		...
		if (curSong.toLowerCase() == 'alter-ego')
		{
				switch(curBeat)
				{
					//alter ego eventos
					case 3 | 7 | 11 | 15 | 19 | 23 | 27 | 31 | 35 | 39 | 43 | 47 | 51 | 55 | 59 | 60 | 62 | 63 | 128 | 132 | 144 | 148 | 160 | 188 | 189 | 190 | 191 | 257 | 258 | 259 | 332 | 334 | 335:
						FlxTween.tween(FlxG.camera, {zoom: 1.4}, 0.4);
					case 32 | 48 | 64 | 129 | 133 | 145 | 149 | 192 | 260 | 272:
						FlxG.camera.flash(FlxColor.WHITE, 0.5, false);
					case 344:
						FlxG.camera.flash(FlxColor.WHITE, 1, false);
					case 256:
						FlxTween.tween(FlxG.camera, {zoom: 2}, 0.4);
					case 314 | 330:
						FlxTween.tween(FlxG.camera, {zoom: 1.1}, 0.4);
					case 336 | 337 | 338 | 339:
						FlxTween.tween(FlxG.camera, {zoom: 1.4}, 0.4);
		   }
		}
