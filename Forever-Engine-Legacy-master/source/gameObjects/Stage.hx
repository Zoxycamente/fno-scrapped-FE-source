	...
	public function new(curStage)
	{
		...
		switch (curStage)
		{
			case 'escondidoBG':
				...
				var bg:FNFSprite = new FNFSprite(-600, -400).loadGraphic(Paths.image('backgrounds/' + curStage + '/oculto'));
				add(bg);
			case 'bobinho':
				PlayState.defaultCamZoom = 0.9;
				
				var bobinhoBG:FlxSprite = new FlxSprite(-600,
					-400).makeGraphic(FlxG.width, FlxG.height, FlxColor.WHITE); // era só fazer um código pra ficar branco lol
				bobinhoBG.scrollFactor.set(0.9, 0.9);
				add(bobinhoBG);
			default:
				PlayState.defaultCamZoom = 0.9;
				curStage = 'BGsPrincipais';

				var bg:FNFSprite = new FNFSprite(-600, -400).loadGraphic(Paths.image('backgrounds/' + curStage + '/ocultoBG'));
				add(bg);
		}
	}
